...
import { getJobPositionById, updateJobPositionById } from 'apiSdk/job-positions';
...
function JobPositionEditPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<JobPositionInterface>(
  ...
  const handleSubmit = async (values: JobPositionInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateJobPositionById(id, values);
      mutate(updated);
      resetForm();
