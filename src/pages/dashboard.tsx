...
import { getJobPositions } from 'apiSdk/job-positions';
...
function DashboardPage() {
  ...
  const jobPositionStat = useSWR<PaginatedInterface<JobPositionInterface>>(
    () => `/job-positions`,
    () => getJobPositions({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Job Positions',
      result: jobPositionStat?.data?.totalCount,
      isLoading: jobPositionStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Attendances',
